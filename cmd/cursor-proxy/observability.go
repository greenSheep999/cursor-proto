...
// request's tools[] array. On the 3.10 line the server-tool reject
// path (v0.2.2 on main) doesn't exist, so we don't need to filter
// those out separately — Anthropic server tools carry a `type` but
// no `name` field with the actual tool identifier, so they'd be
// dropped here as blank anyway.
